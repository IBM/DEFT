...
from SQuAD.deft_trainer import QuestionAnsweringSeq2SeqTrainerAdapter
...
class ModelArguments:
    ...
    eps: float = field(
        ...
        metadata={
            "help": (
                "Epsilon for the l0 approximation"
            )
        },
    )
    sparse_obj: bool = field(
        default=True,
        metadata={
            "help": (
                "Whether to use the sparse objective or not"
            )
        },
    )
    red_factor: int = field(
        default=16,
        metadata={
            "help": (
                "The reduction factor for the adapter"
            )
        },
    )
...
def main():
    ...
    metric = evaluate.load("squad_v2" if data_args.version_2_with_negative else "squad",experiment_id = f"adapter_squad1_{str(training_args.seed)}")
    ...
    eps = model_args.eps
    sparse_obj = model_args.sparse_obj

    print("Epsilon: ",eps)
    print("Sparse Objective:  ",sparse_obj)

    adapter_args.train_adapter = True
    print(adapter_args)
    adapter__kwargs = {}
    adapter__kwargs["non_linearity"]= "relu"
    adapter__kwargs["reduction_factor"]= model_args.red_factor


    
    setup_adapter_training(model, adapter_args, adapter_name="squad",adapter_config_kwargs=adapter__kwargs)
    ...
    output_hook_enc = OutputHook()
    output_hook_dec = OutputHook()

        
    act_layers_to_save = "DenseReluDense.act"

    # Register hooks to capture the activation outputs
    for name, module in model.named_modules():
        if act_layers_to_save in name and "encoder" in name:
            hook = module.register_forward_hook(
                output_hook_enc
            )
        if act_layers_to_save in name and "decoder" in name:
            hook = module.register_forward_hook(
                output_hook_dec
            )
    ...
    print("Trainable %: ",trainable_params/total_params)


    # Initialize our Trainer
    trainer = QuestionAnsweringSeq2SeqTrainerAdapter(
        eps = eps , 
        Output_hook_enc= output_hook_enc,
        Output_hook_dec = output_hook_dec,
        sparsity_loss = sparse_obj,
        model=model,
        args=training_args,
    ...
    if training_args.do_eval:
        ...
        metrics,mean_zeros = trainer.evaluate(max_length=max_length, num_beams=num_beams, metric_key_prefix="eval")
        print(mean_zeros)
