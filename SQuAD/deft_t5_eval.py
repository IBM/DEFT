...
from t5_eval import QuestionAnsweringSeq2SeqTrainerAdapter
...
class ModelArguments:
    ...
    chkpt_path: str = field(
        default=None,
        metadata={"help": "Path to the checkpoint to load the model from."}
    )
...
def main():
    ...
    adapter__kwargs["reduction_factor"]= model_args.reduction_factor

    # NO LEAVEOUT
    # adapter__kwargs["leave_out"] = []
    
    setup_adapter_training(model, adapter_args, adapter_name="squad",adapter_config_kwargs=adapter__kwargs)
    
    chkpt = model_args.chkpt_path 
    x = torch.load(chkpt)
