...
from transformers import (AdapterConfig, AutoAdapterModel, AutoConfig,
                          AutoModelForSequenceClassification, AutoTokenizer,
                          EvalPrediction, PfeifferConfig, PretrainedConfig,
                          default_data_collator,LoRAConfig,
...
lora_config = LoRAConfig(r=args.rank, alpha=args.alpha,dropout=args.dropout)
# Add a new adapter
model.add_adapter(args.task,config = lora_config)
...
print("ADAPTER_ARGS: ",lora_config)
...
for epoch in range(num_epochs):
    ...
    if sparse_obj:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_lora_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_lora_{epoch}", from_pt=True)
    else:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_lora_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_lora_{epoch}", from_pt=True)
