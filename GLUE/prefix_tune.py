...
peft_config = PrefixTuningConfig(task_type="SEQ_CLS", num_virtual_tokens=args.num_virtual_tokens)
...
for epoch in range(num_epochs):
    ...
    if sparse_obj:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_prefix_t_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_prefix_t_{epoch}", from_pt=True)
    else:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_prefix_t_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_prefix_t_{epoch}", from_pt=True)
