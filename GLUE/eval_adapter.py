...
parser.add_argument("--path_chkpt", type=str, default="")
...
act_layers_to_save = "intermediate_act_fn"
for name, module in model.named_modules():
...
if args.task is not None:
    metric = evaluate.load("glue", args.task,experiment_id= f"{args.task}_{args.seed}__{args.sparse_obj}_l0_adapter_")
elif is_regression:
    metric = evaluate.load("mse",experiment_id= f"{args.task}_{args.seed}__{args.sparse_obj}_l0_adapter_")
else:
    metric = evaluate.load("accuracy",experiment_id= f"{args.task}_{args.seed}__{args.sparse_obj}_l0_adapter_")
...
print({"density":mean_outs.mean()*100})
