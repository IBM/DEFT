...
from transformers import (AutoAdapterModel, AutoConfig,
                           AutoTokenizer,
                          EvalPrediction, PfeifferConfig, PretrainedConfig,
...
parser.add_argument("--adapter_config", type=str, default="pfeiffer",help="lohfink-rossi-leaveout|pfeiffer")
...
model = AutoAdapterModel.from_pretrained(model_name_or_path,config=config,cache_dir =args.cache_dir)

adapter_conf = PfeifferConfig(non_linearity=args.non_linearity,reduction_factor=args.reduction_factor)
# Add a new adapter
model.add_adapter(args.task,config = adapter_conf)
...
print("ADAPTER_ARGS: ",adapter_conf)
...
for epoch in range(num_epochs):
    ...
    if sparse_obj:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_adapter_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__SPARSE_{weight_l}_{SEED}_{task}_{args.eps}/{task}_sparse_adapter_{epoch}", from_pt=True)
    else:
        model.base_model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_adapter_basemodel_{epoch}", from_pt=True)
        model.save_pretrained(f"/{args.model_name_or_path}/GELU__{weight_l}_{SEED}_{task}_{args.eps}/{task}_normal_adapter_{epoch}", from_pt=True)
